...
def genTextImg():
    ...
    for size in font_sizes:
        ...
        tmp = []
        tmp.append(ImageFont.truetype('data/fonts/more_font//仿宋_GB2312.ttf', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//华文隶书.TTF', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//宋体_GB18030+%26+新宋体_GB18030.ttc', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//微软vista黑体.ttf', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//方正楷体GBK.ttf', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//方正隶书简体.ttf', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//楷体_GB2312.ttf', size))
        tmp.append(ImageFont.truetype('data/fonts/more_font//造字工房尚黑G0v1纤细长体.otf', size))
        fonts.append(tmp)
    ...
    index = 0
    print("--------------------")
    txtFiles = list(filter(lambda x: x.endswith('.txt') or x.endswith('.TXT'), txtFiles))
    print(txtFiles)
    print("--------------------")
    for file in txtFiles:
        fullPath = os.path.join(txtPath, file)
        ##输入文档
